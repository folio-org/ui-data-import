...
export const getWrapperSourceLink = (key, resourceLimit = PER_REQUEST_LIMIT) => {
  const WRAPPER_SOURCE_LINKS = {
    PREFIXES: `/orders/settings/prefixes?limit=${resourceLimit}&query=${FIND_ALL_CQL} sortby name`,
    SUFFIXES: `/orders/settings/suffixes?limit=${resourceLimit}&query=${FIND_ALL_CQL} sortby name`,
